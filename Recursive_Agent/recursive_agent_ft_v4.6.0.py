...
class RecursiveAgentFT(LazyMonitorMixin):
    ...
    def __init__(
        ...
        *,
        ...
        # Swirl + Density
        self.swirl = AgentSwirlModule(swirl_size=16)
        self.density = MotifDensityTracker()

        # Phase behavior
        self.phase_mode: str = "delay"
    ...
    def export_feedback_packet(self) -> Dict[str, Any]:
        """Lightweight runtime stats for field observers (RFC‑0005 §4, RFC‑0007 extension)."""
        self.FEEDBACK_EXPORT.labels(agent_id=self.agent_id).inc()

        # Snapshot state
        swirl_hash = self.swirl.compute_swirl_hash()
        density_snapshot = self.density.snapshot()
        top_density = sorted(density_snapshot.items(), key=lambda x: -x[1])[:5]

        # Compute adaptive phase tier
        C_i = compute_coherence_potential(self._reward_ema, self.entropy_slope)
        tier = "low" if C_i < 0.8 else "med" if C_i < 2.5 else "high"

        # Symbolic motif classification
        SYMBOLIC_PHASE_MAP = {
            "bind": "ψ‑bind",
            "spar": "ψ‑spar",
            "null": "ψ‑null",
    ...
    def _emit_tick(self, motifs: List[str], *, stage: str = "E2b") -> QuantumTickV2:
        ...
        field_sig = self._resolve_field(motif_id)

        # ⟳ Update state
        self.swirl.update_swirl(motif_id)
        self.density.update_density(motif_id)
        ...
        # ⤿ Extend tick metadata
        tick.extensions.update({
            "coherence_potential": C_i,
            "swirl_vector": swirl_hash,
        })
