"""
logical_agent_at_update_0001.py · v1.0.3 — hot‎‑patch for Noor Logical/Recursive agents
    ...
    if not _check_version(version_tuple, _MIN_LOGICAL_VERSION):
        ...
        if not hasattr(LogicalAgentAT, "_orig_complete_triad"):
            ...
            def export_motif_bundle(self) -> dict[str, Any]:
                """Exports a snapshot of motif state for external tools or debugging."""
                try:
                    motifs = list(getattr(self, "motif_embeddings", {}).keys())
                    triads = list(getattr(self, "_triad_links", {}).items()) if hasattr(self, "_triad_links") else []
                    return {
