...
# ──────────────────────────────────────────────────────────────
# Dynamic‑Feature‑Flag Extension  (completes v3.1.0 spec)
# ──────────────────────────────────────────────────────────────
...
try:
    ...
    class _FeatStub:                                    # noqa: D401
        def __getattr__(self, _):                       # noqa: ANN001
            return lambda *a, **k: None                 # noqa: D401

    _FEATURE_TOGGLE_COUNTER = _FeatStub()               # type: ignore

# ---------------------------------------------------------------------------
# Flag whitelist ‑ only these names are reversible at runtime
# ---------------------------------------------------------------------------
_DYNAMIC_FLAGS = {
...
# ---------------------------------------------------------------------------
# Helper mix‑ins (added via monkey‑patch so we don’t disturb earlier code)
# ---------------------------------------------------------------------------
def _init_dynamic_flags(self):
    """Internal: initialise flag‑state mirror & audit trail (thread‑safe)."""
    self._flag_state: Dict[str, bool] = {k: getattr(self, k) for k in _DYNAMIC_FLAGS}
    self._flag_audit: List[tuple[int, str, bool, str]] = []  # (ts, flag, val, reason)

def _set_feature(self, name: str, value: bool, *, reason: str = "") -> None:
    """
    Thread‑safe runtime toggle for dynamic flags.

    Raises
    ------
    ValueError
        If `name` is not whitelisted.
    """
...
def _get_feature(self, name: str) -> bool:
    """Return current value of a dynamic flag (raises if unknown)."""
    if name not in _DYNAMIC_FLAGS:
        raise ValueError(f"'{name}' is not a dynamic flag")
    return getattr(self, name)

def _list_dynamic_flags(self) -> Dict[str, bool]:
    """Return a `{flag: value}` mapping for all dynamic flags."""
    return {k: getattr(self, k) for k in _DYNAMIC_FLAGS}

# ---------------------------------------------------------------------------
# Inject helpers into LogicalAgentAT if they aren’t present already
# ---------------------------------------------------------------------------
for _fname, _impl in {
    "_init_dynamic_flags": _init_dynamic_flags,
    "set_feature":        _set_feature,     # overrides simple stub earlier
...
# Ensure existing __init__ runs the dynamic‑flag initialiser exactly once
if not hasattr(LogicalAgentAT, "_flag_state"):
