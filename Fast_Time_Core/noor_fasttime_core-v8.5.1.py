...
class NoorFastTimeCore:
    ...
    def _ingest_tick(self, tick: QuantumTick, change_id: Optional[MotifChangeID] = None) -> None:
        ...
        # Track gate_id if present
        gate_id = getattr(tick, "gate_id", None)
        if gate_id is not None:
            if not hasattr(self, "_gate_histogram"):
                self._gate_histogram = _PyCounter()
    ...
    def _compute_gate_heatmap(self) -> Dict[int, int]:
        if hasattr(self, "_gate_histogram"):
