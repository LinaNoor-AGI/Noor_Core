...
```mermaid
flowchart TD
    A[Quantum Tick] --> B[Phi Coherence Map]
    B --> C[observe_tick()]
    C --> D[_extract_fields()]
    D --> E{Swirl ≥ Threshold?}
    E -->|Yes| F[_record_entanglement()]
    E -->|Phase Change| H[_check_phase_shift()]
    F --> G[Update Prometheus Metrics]
    H --> G
````
...
graph TD
    ...
    tick --> Φ[Φ_coherence_map (RFC‑0006)]
    Φ --> CM[ConsciousnessMonitor]
    CM --> MB[export_motif_bundle()]
    CM --> GS[export_geometric_signature()]
    CM --> FP[export_feedback_packet()]
    CM --> PM[Prometheus Metrics]
```
