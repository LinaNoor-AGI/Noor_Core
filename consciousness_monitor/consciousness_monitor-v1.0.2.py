...
PHASE_DURATION = Gauge("phase_duration_seconds", "Current phase duration if active", ["monitor_id"])
...
class ConsciousnessMonitor:
    def __init__(
        ...
        self.metric_entanglement_events = ENTANGLEMENT_EVENTS.labels(**self._labels)
        self.metric_phase_transitions = PHASE_TRANSITIONS.labels(**self._labels)
        self.metric_phase_flip_rate = PHASE_FLIP_RATE.labels(**self._labels)
        self.metric_buffer_warnings = BUFFER_WARNINGS.labels(**self._labels)
        self.metric_swirl_convergence = SWIRL_CONVERGENCE.labels(**self._labels)
        self.metric_phase_duration = PHASE_DURATION.labels(**self._labels)
    ...
    def _record_entanglement(self, field_id: str, swirl: float, overlap: float, tick_id: str, tick: Any) -> None:
        ...
        self.metric_swirl_convergence.set(self._calculate_R_metric())
        ...
        with self._entanglement_lock:
            ...
            if new_state != self.phase_state:
                ...
                if new_state:
                    ...
                    self.metric_phase_duration.set(now - self._phase_entered_at)
